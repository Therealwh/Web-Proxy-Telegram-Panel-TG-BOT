...
// --- QR-страница клиента ---
router.get('/qr/:id', publicLimiter, (req, res) => {
...
// --- Публичные QR-изображения (для страницы /qr/:id) ---
router.get('/qrimg/:id/:type.png', publicLimiter, async (req, res) => {
    const client = db.prepare("SELECT * FROM clients WHERE id = ? AND status = 'active'").get(req.params.id);
