 ...
 * @param {boolean|string} [reload='instant'] - режим runtime reload:
 *   false = только сохранить в файл, 'instant' | 'drain' = применить сразу
 */
const patchConfig = (patch, reload = 'instant') =>
    apiRequest('PATCH', `/v1/config${reload ? `?reload=${reload}` : ''}`, patch);
