...
test('mtprotoLink: формат tg://proxy с dd-префиксом (Secure)', () => {
    const secret = 'b'.repeat(32);
    const link = mtprotoLink(secret, 'www.cloudflare.com'); // второй аргумент игнорируется
    assert.equal(
        link,
        `tg://proxy?server=proxy.example.com&port=8443&secret=dd${secret}`
    );
});
