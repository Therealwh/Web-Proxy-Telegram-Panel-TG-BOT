...
# --- Версия панели: из файла VERSION (реальный код на диске) ---
panel_current="$(cat /opt/tggate/VERSION 2>/dev/null | tr -d '[:space:]')"
...
if [[ ${VERBOSE} -eq 1 ]]; then
    echo "Панель:  текущая ${panel_current:-?} → последняя ${panel_latest:-н/д}"
